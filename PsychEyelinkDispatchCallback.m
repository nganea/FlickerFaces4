...
%
% History:
%  5. 4.2023    Randomly select one of 3 animation targets for each corner
%                   of the screen when calibrating. (Natasa Ganea)
%
...
switch eyecmd
    ...
    case 5  % Draw Cal Target
        ...
        if isfield(el,'changeCalAnimation') == 1 && el.changeCalAnimation == 1
            ...
            if isfield(el,'calAnimationList') == 1 && ~isempty(el.calAnimationList)
                ...
                el.calAnimationTargetFile = [pwd '/' calMovieName];
    ...
    case 10 % Erase Cal Target
        ...
        if strcmpi(el.calTargetType, 'video') && ~isempty(eyelinkanimationtarget)
            texkill=Screen('GetMovieImage', eyewin, eyelinkanimationtarget.movie, 0);
...
dontsync = 0;
Screen('Flip', eyewin, [], dontsync, 1);   % Show it
